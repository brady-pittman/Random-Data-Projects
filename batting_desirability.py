...
data = pd.read_csv('C:\\Users\\602353\\Downloads\\FanGraphs Leaderboard (batters).csv')
...
league_average_pa = data_no_team['PA'].mean()
pa_deviation = data_no_team['PA'] - league_average_pa
data_no_team['Desirability Rate'] = (data_no_team['BB%'] + data_no_team['Barrel%'] + data_no_team['Hard%']) - (data_no_team['K%'] + data_no_team['GB%'] + data_no_team['Soft%']) + (0.04 * pa_deviation).round(1)
average_rate = data_no_team['Desirability Rate'].mean().round(1)

fig, ax = plt.subplots(figsize=(10, 8))
scatter = ax.scatter(data_no_team['Desirability Rate'], data_no_team['PA'], alpha=0.6, c='royalblue', edgecolors='gray')

z = np.polyfit(data_no_team['Desirability Rate'], data_no_team['PA'], 1)
p = np.poly1d(z)
...
ax.set_ylabel('Plate Appearances', fontsize=12)
ax.set_title('PA vs. Desirability Index', fontsize=14)
...
for i, player in top_3.iterrows():
    ax.annotate(player['Name'], (player['Desirability Rate'], player['PA']), xytext=(10, -10),
...
ax.scatter(average_rate, league_average_pa, color='gold', marker='o', s=100, edgecolors='black', label='Average')
ax.annotate(f'Average: {average_rate}', (average_rate, league_average_pa), xytext=(10, -10), textcoords='offset points',
             color='black', fontsize=12)

ax.legend(loc='upper left', fontsize=10)

ax.set_xlim([data_no_team['Desirability Rate'].min() - 1, data_no_team['Desirability Rate'].max() + 1])
ax.set_ylim([data_no_team['PA'].min() - 100, data_no_team['PA'].max() + 100])
